...
abstract: "Although the study of unidentified aeriel phenomena (UAP) has only recently captured the interest of the U.S. Congress, there is a long tradition of attempting to use data and the scientific method to explain it. Using large databases of observations, the natural first step is to map reported sightings to locate potential hot spots. However, maps of UAP observations tend to reproduce the population distribution of the territory. That is, the biggest clusters coincide with the biggest populations. This is not informative. This paper uses simple linear regression to quantify the effect of population and land area on the number of reported UAP. The model errors are then interpreted as the number of sightings in excess of the pure population/area effect. This generates a list of true hot spots, which may suggest novel hypotheses to be tested or sensible placements for electronic sensor systems.</br></br><b>Author e-mail:</b> <a href='mailto:benhorvath@gmail.com'>benhorvath@gmail.com</a></br><b>Github repository</b>: <a href='https://github.com/benhorvath/nuforc_stats/'>benhorvath/nuforc_stats</a></br></br></br>"
---
...
The `R` code for this project is stored in the Github repository [benhorvath/nuforc_stats](https://github.com/benhorvath/nuforc_stats/). Contact the author at [benhorvath@gmail.com](mailto:benhorvath@gmail.com) for any data requests.


# Observational capacity

The simplest model of a territory's UAP observations abstracts from the particulars of time and geography. It assumes that every person in a given territory has an equal chance of observing UAP, every portion of land within the territory has an equal chance of 'manifesting' UAP, and that these probabilities do not change over time. It additional assumes that _rates of reporting_ do not vary across populations. Although these assumptions are probably wrong, they can be provisionally accepted for the purposes of this initial work.

Comparing the maps above, it is clear that population is a major factor explaining the varying levels of observation between locales. All else being equal, a territory with a larger population has more people observing the sky. That is, larger cities have more _observational capacity_, a greater probability of observing and reporting aerial phenomena.

However, the land area of a territory also must be taken into account. At some point, the marginal increase of additional population drops to zero---the additional population will be 'covering' a part of the sky that is already well-observed, probably by multiple people simultaneously. There is no benefit to additional population. This 'saturation point' depends on the land area of the territory. The larger the territory, the more population it takes before the marginal benefit of an additional set of eyes reaches zero. That is, population density being equal, we expect the number of sightings to be larger in larger territories. More UAP will be observed in a one hundred square mile territory with a given population density, than in a one square mile territory with the same density. Similarly, increasing the number of people in a territory increases the amount of sky that can be observed. And the larger the portion of the sky that is 'covered', the more UAP will be reported. 
...
where $\beta_j$ represents a scalar controlling the relationship between an independent variable $x_j$ and $y$. Such a model is easily fit from empirical data via the least squares procedure, available in the statistical software `R` (R Core Team 2020).
...
```{r, echo=FALSE, fig.show='hold', out.width='50%', fig.cap='Graphical summaries of the variables under study. Left shows the raw variables; right shows the natural log-transformed variables. The large decimals refer to the Pearson correlation between the variables, with asterisks marking the statistical significance.'}
...
A linear regression model is fit on the natural logs of land area (square miles), population density (persons per square mile), and the interaction between the two. These two variables were chosen among those available because they were not highly correlated, as required by the assumptions of regression. The full software output is below:
...
Interpreting the fit parameters is complicated by the model's structure. The interaction term necessarily implies that the effect of density on the number of UAP observations varies with the size of the land area, and vice versa. A graphical display communicates the model more succinctly:
...
Utah is often considered an area where UAP concentrate (Salisbury 2010). Because of this reputation, it is worth examining more closely. After accounting for area and population, the top ten Utah hot spots are:
...
The plots below show the fitted model model generally conforms to the standard assumptions of linear regression. There is no clear pattern when the predictions are plotted against the residuals, although the artifacts of only having positive numbers for $y$ are observable. The Q-Q plot shows some strong outliers. These are the handful of American cities with truly exceptional populations, such a New York City and Los Angeles. There is no apparent relationship between the order of the observations, and the residuals are approximately normal with a central tendency near zero.

```{r, echo=FALSE, fig.align='center', fig.cap='Residual diagnostic plots.'}
plot_resids
```

Errors do not vary by longitude or latitude:
